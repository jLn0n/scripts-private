-- fork of https://github.com/thedoomed/Cheat-Engine/blob/master/executor.lua
-- might be maintained rarely
assert(_VERSION ~= "5.3", "Lua 5.3 expected");

script_source = [[
	script.Name, script.Parent = "", game:GetService("CoreGui").RobloxGui
...
rbx.execute_closure = function(closure)
	...
	rbx.functions.sc_runscript(rbx.script_context, rbx.local_script, rbx.functions.get_instance_source(rbx.local_script), 8, 0, 0, 0, 0, 0, 0, 0, 0);
...
rbx.start = function()
	rbx.execute_script("spawn(function()\n" .. script_source .. "\nend)");
	_ENV.execute_closure = rbx.execute_closure
	--retcheck.flush();
	--fremote.flush();
end
