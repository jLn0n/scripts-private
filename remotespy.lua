...
local synHL_Lib = loadstring(game:HttpGetAsync("https://raw.githubusercontent.com/jLn0n/created-scripts-public/main/libraries/lua-syntax-highlighting.lua", true))()
local Source = synHL_Lib.new({
	...
	ZIndex = 1,
	SyntaxColors = {
		builtin = Color3.fromRGB(200, 25, 25),
		comment = Color3.fromRGB(75, 185, 75),
		iden = Color3.fromRGB(215, 215, 215),
		keyword = Color3.fromRGB(25, 255, 0),
		number = Color3.fromRGB(90, 90, 90),
		operator = Color3.fromRGB(90, 90, 90),
		string = Color3.fromRGB(255, 132, 0)
	}
})
...
local get_namecall_dump = function(scriptArg, object, args)
	local result_freturn
	if object:IsA("RemoteFunction") then
		local f_return_ = {pcall(object.InvokeServer, object, table.unpack(args))}
		f_return = {select(2, table.unpack(f_return_))}

		if #f_return ~= 0 then
			result_freturn = Table_TS(f_return_)
		end
	end
	namecall_dump[#namecall_dump + 1] = {
		cScript = namecall_script(object, object:IsA("RemoveEvent") and "FireServer" or "InvokeServer", args),
		caller = scriptArg,
		object = object,
		method = (object:IsA("RemoveEvent") and "FireServer" or "InvokeServer"),
		f_return = tostring(result_freturn)
	}
end
...
game_namecall = hookmetamethod(game, "__namecall", function(self, ...)
	...
	local args = {...}
	if self.Name ~= "CharacterSoundEvent" and string.match(string.lower(namecallMethod), "server") and spy_enabled then
		get_namecall_dump(getcallingscript(), self, args)
	end
