...
local GetType = function(object)
	local Types = {
		...
		BrickColor = function()
			return string.format("BrickColor.new(%s)", tostring(object))
		end,
		Color3 = function()
...
local hide = function()
	...
	pos_frame(BG, UDim2.new(0, 100, 0, -26))
...
local namecall_script = function(object, method, ...)
	local scriptVar = string.format("-- Script generated by R2Sv2\n-- R2Sv2 developed by Luckyxero\n-- Modified by jLn0n#1464\n-- Remote Path: %s\n\n", GetPath(object))
	local remoteArgs = {}
	for iterator, value in pairs({...}) do
		scriptVar = scriptVar .. string.format("local A_%s = %s\n", iterator, (type(value) == "table" and Table_TS(value) or GetType(value)))
		table.insert(remoteArgs, "A_" .. iterator)
	end
	scriptVar = scriptVar .. string.format("local Event = %s\n\n", GetPath(object))
	scriptVar = scriptVar .. string.format("Event:%s(%s)", method, table.concat(remoteArgs, ", "))
	return scriptVar
end
...
local log_remote = function(tableArg)
	...
	B.MouseButton1Down:Connect(function()
		dump_script(tableArg.script)
		g_caller = tableArg.caller
		f_return = tableArg.f_return == nil and tableArg.object.Name .. " is not RemoteFunction" or tableArg.f_return
	end)
end
...
local on_namecall = function(object, ...)
	...
	if object.Name ~= "CharacterSoundEvent" and method:match("Server") and spy_enabled == true then
		get_namecall_dump(getfenv(2).script, object, unpack(args))
	end
